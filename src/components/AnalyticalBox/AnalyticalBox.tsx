...
const IconContainer = styled(
    ...
    color: isPaletteColor(color) ? theme.palette[color].main : defaultColor(theme),
    // @ts-ignore TODO remove ts-ignore. This should work...
    backgroundColor: alpha(isPaletteColor(color) ? theme.palette[color].main : defaultColor(theme), theme.palette.action.selectedOpacity),
    display: 'flex',
    alignItems: 'center',
    ...
    borderRadius: typeof theme.shape.borderRadius === 'number' ? theme.shape.borderRadius * 2 : '10px',
}))
...
const defaultColor = (theme: Theme): string => (
